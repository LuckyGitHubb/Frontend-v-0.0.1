...
function BranchList() {
  ...
  const deleteBranch = async (id) => {
    try {
      await globalApi("POST", { id }, Branch_API.DELETE);
      fetchBranches();
    } catch (error) {
  ...
  const filterBranch = (value) => {
    if (!value) return fetchBranches();

    const searchValue = value.toLowerCase();
    const filtered = originalData.filter(branch =>
      Object.values(branch).some(field =>
  ...
  const exportData = () => (selectedRows.length > 0 ? selectedRows : originalData);
  ...
  return (
    <MainDiv>
      <Section>
        <h2 className="text-2xl font-bold text-[#4c4c4c]">Branch List</h2>
        <Button onClick={() => navigate("/branch/add", { state: { mode: "add" } })}>
          Add Branch
        </Button>
      </Section>
      ...
      <div className="flex justify-between items-center">
        <div className="p-2 w-1/3 mt-10 mb-4 flex border-b border-[#cdcdcd]">
          <input
            ...
            onChange={(e) => filterBranch(e.target.value)}
          />
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gradient-to-r from-blue-400 to-blue-600 text-white">
            <th className="p-4">
              <input type="checkbox" checked={selectAllPage} onChange={handleSelectAllPage} />
            </th>
              ...
              >
                ...
                <td className="p-4 grid grid-cols-3">
                  <FaEye
                    ...
                    onClick={() => navigate("/branch/add", { state: { mode: "view", data: item } })}
                  />

                  <MdEdit
                    className="text-yellow-500 cursor-pointer"
                    onClick={() => navigate("/branch/add", { state: { mode: "edit", data: item } })}
                  />
      ...
      <div className="flex justify-between items-center p-4">
        ...
        <div className="flex items-center">
          <IoIosArrowBack onClick={handlePrevious} className="cursor-pointer" />
