...
import { Purchase_API } from "../../../services/apiRoutes";
...
function PurchaseList() {
  ...
  const deletePurchase = async (id) => {
    try {
      await globalApi("POST", { id }, Purchase_API.DELETE);
      fetchPurchases();
    } catch (error) {
   ...
   *   Search
   ==============================*/
  const filterPurchase = (value) => {
    if (!value) return fetchPurchases();
  ...
  return (
    <MainDiv>
      <Section>
        <h2 className="text-2xl font-bold text-[#4c4c4c]">Purchase List</h2>
        <Button
          onClick={() =>
            navigate("/purchase/add", { state: { mode: "add" } })
          }
        >
          Add Purchase
        </Button>
          ...
          {data.length > 0 ? (
            data.map((item, index) => (
              ...
              >
                ...
                <td className="p-4">{item?.batchNo}</td>
